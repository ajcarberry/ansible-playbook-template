...
#
# Syntax: ./docker-debian.sh [enable non-root docker socket access flag] [source socket] [target socket] [non-root user] [CLI version]

ENABLE_NONROOT_DOCKER=${1:-"true"}
SOURCE_SOCKET=${2:-"/var/run/docker-host.sock"}
TARGET_SOCKET=${3:-"/var/run/docker.sock"}
USERNAME=${4:-"automatic"}
DOCKER_VERSION=${5:-"latest"}
...
# Set up the necessary apt repos (Docker's)
...
if type docker-compose > /dev/null 2>&1; then
    ...
    if [ "${TARGET_COMPOSE_ARCH}" != "x86_64" ]; then
        ...
        if ! dpkg -s python3-minimal python3-pip libffi-dev python3-venv > /dev/null 2>&1; then
            apt_get_update_if_needed
            apt-get -y install python3-minimal python3-pip libffi-dev python3-venv --no-install-recommends
        fi
        ...
        if ! type pipx > /dev/null 2>&1; then
            pip3 install --disable-pip-version-check --no-cache-dir --no-warn-script-location --user pipx
...
if ! dpkg -s socat > /dev/null 2>&1; then
    apt_get_update_if_needed
    apt-get -y install socat --no-install-recommends
fi
